...
import gc.grivyzom.AnforaXP.data.StorageEngine;
import gc.grivyzom.AnforaXP.data.YamlStorage;
...
public class AutoSaveTask extends BukkitRunnable {
    ...
    public AutoSaveTask(AnforaMain plugin, PlayerDataManager playerDataManager, AnforaDataManager anforaDataManager, StorageEngine storageEngine) {
        this.plugin = plugin;
    ...
    public void run() {
        try {
            ...
            final int playersSaved = playerDataManager != null ? playerDataManager.getCacheSize() : 0;
            final int anforasSaved = anforaDataManager != null ? anforaDataManager.getCacheSize() : 0;

            if (storageEngine instanceof YamlStorage) {
                // Para YAML, llamamos a un método que guarda todo de una vez.
                ((YamlStorage) storageEngine).saveAllToDisk();
            } else {
                // Para bases de datos, guardamos los datos cacheados.
            ...
            Bukkit.getScheduler().runTask(plugin, () -> {
                plugin.getLogger().info(String.format(
                    "Auto-save completado: %d jugadores, %d ánforas procesadas en %dms",
